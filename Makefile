...
ifeq ($(strip $(area)),)
  ...
  data_files := $(shell find $(DATA_DIR) -name '*.osm.pbf' 2>/dev/null)
  ifneq ($(word 2,$(data_files)),)
    ...
    ifeq ($(word 1,$(data_files)),)
      ...
      area := $(patsubst $(DATA_DIR)/%.osm.pbf,%,$(data_files))
      ...
      ifneq ($(area),$(area:-latest=))
        $(shell mv "$(DATA_DIR)/$(area).osm.pbf" "$(DATA_DIR)/$(area:-latest=).osm.pbf")
        area := $(area:-latest=)
        $(warning ATTENTION: File $(DATA_DIR)/$(area)-latest.osm.pbf was renamed to $(area).osm.pbf.)
        AREA_INFO := Detected area=$(area) based on finding a '$(DATA_DIR)/$(area)-latest.osm.pbf' file - renamed to '$(area).osm.pbf'. Use 'area' parameter or environment variable to override.
      else
        AREA_INFO := Detected area=$(area) based on finding a '$(DATA_DIR)/$(area).osm.pbf' file. Use 'area' parameter or environment variable to override.
      endif
    endif
  endif
endif
...
export BORDERS_CLEANUP_FILE = /import/borders/$(area).cleanup.pbf
export BORDERS_PBF_FILE = /import/borders/$(area).filtered.pbf
export BORDERS_CSV_FILE = /import/borders/$(area).lines.csv
...
ifeq ($(strip $(DIFF_MODE)),true)
  ...
  export IMPOSM_CONFIG_FILE = /import/$(area).repl.json
endif

# Load area-specific bbox file that gets generated by the download-osm --bbox
AREA_BBOX_FILE ?= /import/$(area).bbox
AREA_BBOX_FILE1 ?= $(DATA_DIR)/$(area).bbox
ifneq (,$(wildcard $(AREA_BBOX_FILE1)))
  cat := $(if $(filter $(OS),Windows_NT),type,cat)
  BBOX := $(shell $(cat) ${AREA_BBOX_FILE1})
  export BBOX
endif
...
.PHONY: clip-borders
clip-borders: start-db-nowait
	$(DOCKER_COMPOSE) run $(DC_OPTS) openmaptiles-tools sh -c \
		'pgwait && import-borders'
...
start-tileserver: init-dirs
	...
	@echo "***********************************************************"
	@echo " "
	docker run $(DC_OPTS) -it --name tileserver-gl -v $(DATA_DIR):/data -p $(TPORT):$(TPORT) maptiler/tileserver-gl --port $(TPORT)
